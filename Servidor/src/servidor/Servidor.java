...
public class Servidor {
    ...
    private static final Scanner scanner = new Scanner(System.in); // Compartido por todos los hilos

    public static void main(String[] args) throws IOException {
        if (args.length < 2) {
            System.out.println("Uso: java Servidor [MaxClientes] [claveServidor]");
            return;
        }
    ...
    private static class ConexionesCliente implements Runnable {
        ...
        public void run() {
            try {
                ...
                // Hilo principal para enviar mensajes al cliente
        ...
        private void cerrarTodosLosClientes() {
            synchronized (listaClientes) {
                for (ConexionesCliente c : new ArrayList<>(listaClientes)) {
